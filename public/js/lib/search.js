...
(function($) {
    ...
    function getSearchCursor(cm, query, pos) {
        ...
        // insensitive search.
        return cm.getSearchCursor(query, pos, typeof query == "string"
    ...
    function parseQuery(query) {
        ...
        return isRE ? new RegExp(isRE[1], isRE[2].indexOf("i") == -1 ? "" : "i")
                : query;
    }
                ...
                .operation(function() {
                    ...
                    if (!cursor.find(rev)) {
                        cursor = getSearchCursor(cm, state.query,
                                rev ? CodeMirror.Pos(cm.lastLine())
                ...
                function(query) {
                    ...
                    dialog(
                            ...
                            function(text) {
                                if (all) {
                                    cm.operation(function() {
                                        for (var cursor = getSearchCursor(cm,
                                                query); cursor.findNext();) {
                                            if (typeof query != "string") {
                                                var match = cm.getRange(
                                                        cursor.from(),
                                                        cursor.to()).match(
                                                        query);
                                                cursor.replace(text.replace(
                                                        /\$(\d)/,
                                                        function(_, i) {
                                                            return match[i];
                                                        }));
                                    ...
                                    clearSearch(cm);
                                    var cursor = getSearchCursor(cm, query, cm
                                            .getCursor());
                                    var advance = function() {
                                        ...
                                        if (!(match = cursor.findNext())) {
                                            ...
                                            if (!(match = cursor.findNext())
                                                    || (start
                                                            && cursor.from().line == start.line && cursor
                                                            .from().ch == start.ch))
                                                return;
                                        }
                                        cm.setSelection(cursor.from(), cursor
                                                .to());
                                        cm.scrollIntoView({
                                            from : cursor.from(),
                                            to : cursor.to()
                                        });
                                        confirmDialog(cm, doReplaceConfirm,
                                                "Replace?", [ function() {
                                                    doReplace(match);
                                                }, advance ]);
                                    };
                                    var doReplace = function(match) {
                                        cursor
                                                .replace(typeof query == "string" ? text
                                                        : text
                                                                .replace(
                                                                        /\$(\d)/,
                                                                        function(
